...
namespace current
{
  ...
  void LaplaceProblem<dim>::solve()
  {
    ...
    Timer timer1;
    timer1.start();
    GMRES.solve(A, x, b, preconditioner);
    timer1.stop();
    table_out.add_value("elapsed CPU time in GMRES (sec),",std::to_string(timer1())+",");      
    table_out.add_value("elapsed Wall time in GMRES (sec),",std::to_string(timer1.wall_time())+","); 
    std:: cout << ".." << std::flush;
  }
  ...
  void LaplaceProblem<dim>::run()
  {
    ...
    table_out.set_precision("elapsed CPU time in GMRES (sec),",3);
    table_out.set_precision("elapsed Wall time in GMRES (sec),",3);
